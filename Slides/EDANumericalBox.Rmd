---
title: "Numerical Data:<br>Boxplot and Outliers"
...
For finding quartiles, the `fivenum()` and `quantile()` functions use different methods of calculation, and may produce different results. The `fivenum()` quartiles are the medians of each half of the dataset, also known as Tukey's *hinges*. 
...
What is the five-number number summary for the mileage data? 

```{r}
fivenum(mileage$mpg)
```

Output is a vector of the five values: min, Q<sub>1</sub>, median, Q<sub>3</sub>, max. To get a one of the values, we have to choose it by its location. For example, Q<sub>1</sub> is the second value, or location 2.

```{r}
fivenum(mileage$mpg)[2]
```
...
```{r}
(LF_mpg <- fivenum(mileage$mpg)[2] - 1.5 * IQR(mileage$mpg))
(UF_mpg <- fivenum(mileage$mpg)[4] + 1.5 * IQR(mileage$mpg))
sort(boxplot.stats(mileage$mpg)$out)   # What does boxplot.stats do?
```

## `boxplot.stats` Function

The `$stats` are the ends of the whiskers, the quartiles, and the median. You also get sample size `$n` and outlier values `$out`.
...
print.data.frame(arsenicfences)   # provides a neat presentation
```
...
## Extract Outliers for Analysis

```{r}
LF <- arsenicfences |> pull(LF)   # to get a scalar value
UF <- arsenicfences |> pull(UF)   # to get a scalar value

well_outliers <- wells |> 
  filter(Arsenic < LF | Arsenic > UF) |> 
  arrange(Arsenic)

glimpse(well_outliers)
```

Does this seem to match up with what you see on the boxplot?

## Remove Outliers from the Data

We found fences for the `mpg` variable earlier, and the boxplot seems to show two outliers. 

```{r}
mileage_out_rem <- mileage |> 
  filter(mpg >= LF_mpg, mpg <= UF_mpg)

nrow(mileage_out_rem)
```

Indeed, two outliers were removed. However, removing outliers could create new ones, since the data summaries might change and outliers are relative to the existing data.
...
## Make a Histogram for `erupt_time`

The boxplot obscures that these data are strongly bimodal.
...
However, the mileage data seems unimodal and symmetric.
