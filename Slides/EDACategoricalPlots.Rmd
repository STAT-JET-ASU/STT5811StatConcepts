...
Yiou can load the dataset into `R` from its URL:
...
Faceting creates multiple "panes" in a plot according to some chosen grouping variable. Each pane displays the same kind of plot for a given subgroup. Use [`facet_wrap()`](https://ggplot2.tidyverse.org/reference/facet_wrap.html) or [`facet_grid()`](https://ggplot2.tidyverse.org/reference/facet_grid.html) (more structured) to achieve different configurations.
...
Six plots configured into a faceted grid show `status` for men and women, with one plot (row) for each department.
...
We can switch the orientation of the facets, so departments are columns. What are the pros and cons of this plot versus the one where departments are rows?
...
```{r, echo = FALSE, fig.align="center"}
knitr::include_graphics("https://miro.medium.com/max/500/1*tGVbx_My4et523b8OL2iKQ.png", dpi = 50)
```

Bar plot of languages for GitHub projects with descriptions that contain “cats” from ["How to Meaningfully Play With Data"](https://medium.com/data-science/how-to-meaningfully-play-with-data-f5183fe82539)