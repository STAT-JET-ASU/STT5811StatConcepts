...
knitr::opts_chunk$set(
  message = FALSE,     # suppresses R messages when knitting
  warning = FALSE,     # suppresses R warnings when knitting
  comment = NA         # eliminates leading ## in code output
)
```
...
```{r}
arbuthnot |>
  arrange(total) |> # you could choose to use slice_min() instead
  head(n = 10)      # you can also use slice_head() with arrange()
```
...
> In November of 1703, an extratropical cyclone known as the "Great Storm" struck England. It was the most destructive storm in British history, killing thousands of people and destroying countless homes, as well as hundreds of civilian and naval ships. London was particularly affected by widespread flooding and collapsed chimneys. This likely impacted the number of children born and baptized in London during 1704. The Recruiting Act of 1703 also forced many unemployed but able-bodied men into military. This law was in effect for only one year. 
...
> The trend line has a negative slope, which suggests that when there is a larger number of baptisms, the ratio of boys to girls is smaller. However, there is also a fairly wide scattering (variability) of points around the trend line in the Y direction, so the relationship (if true) is not strong.
...
> NOTE: I made a slight error in the wording of this question, so there are two possible answers; however, both are binomial models, with different conceptualizations of trials.

> We can think of each child as a trial with two possible outcomes: boy or girl. If boys and girls are equally likely, then the probability of each outcome is 0.5. Assuming the children are independent, we could use a binomial model with $n = \text{number of children}$ and $p = 0.5$. Each year has a different $n$, so we would need different models for each, or we could combine the data across all years. 

> NOTE: What is the chance of observing 484,382 boys (or more) out of 938,223 children, if boys and girls are indeed equally likely? Using `pbinom()` in `R`, is about $1.5 \times 10^{-218}$.

> However, if we consider each year a trial, we have a binomial distribution with parameters $n = 82$ and $p = 0.5$, under the assumption that boys and girls are equally likely. In a theoretically perfect world, there would be an equal number of boys and girls each year. If we take randomness into account, then we would expect boys to outnumber girls about half the time (the chance of the counts of boys and girls being exactly equal is vanishingly small for Arbuthnot's annual counts). Arbuthnot observed $82$ successes. We can solve for the probability using the binomial pdf.
