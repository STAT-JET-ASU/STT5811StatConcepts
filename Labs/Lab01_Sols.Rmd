...
> We could think of each child as a trial with two possible outcomes: boy or girl. If boys and girls are equally likely, then the chance of each outcome is 0.5. Assuming children are independent, we could use a binomial model $n = \text{number of children}$ and $p = 0.5$. Each year has a different $n$, we we would need different models, or we could combine the data across all years. 
...
> However, if we talk about the ratios we observed for each year, we have a binomial distribution with parameters $n = 82$, because each year would be one trial, and $p = 0.5$, under the assumption that boys and girls are equally likely. In a theoretically perfect world, there would be an equal number each year. If we take randomness into account, then we would expect boys to outnumber girls about half the time (the probability of the counts of boys and girls being exactly equal is vanishingly small for Arbuthnot's yearly sample sizes, as he himself noted). The random variable $X$ is the number of years in which boys outnumber girls, and Arbuthnot's observed outcome is $x = 82$ successes. We can solve for the probability using the binomial pdf.
